...
const Tag = () => {
    ...
    const showTags = () => {
        return tags.map((tags, index) => {
            return (
                <button onDoubleClick={() => deleteConfirmation(tags.slug)}
    ...
    const showError = () => {
        if (error) {
            return <p className="text-danger">This tag already exists </p>;
        }
    };

    const showRemoved = () => {
        if (removed) {
            return <p className="text-danger">This tag was successfully removed!</p>;
        }
    };
